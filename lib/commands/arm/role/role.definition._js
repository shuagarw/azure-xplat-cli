...
<<<<<<< HEAD
  ...
  role.command('delete [id]')
    ...
    .execute(function (id, options, _) {
    if (!id) {
      return cli.missingArgument('id');
    }
    var subscription = profile.current.getSubscription(options.subscription);
    var client = rbacClients.getAuthzClient(subscription);
    
    var progress = cli.interaction.progress($('Getting role definition to delete'));
    var getResult;
    try {
      getResult = client.roleDefinitions.getById(id, _);
    } finally {
      progress.end();
    }
    ...
    if (getResult.roleDefinition != null) {
      ...
      progress = cli.interaction.progress($('Deleting role definition'));
      var deleteResult;
      try {
      ...
      if (options.passthru) {
        var deletedRole = [];
        deletedRole.push(deleteResult.roleDefinition);
        cli.interaction.formatOutput(deletedRole, function (data) {
          log.table(data, displayARoleDefinition);
        });
      }
    }
    else {
      log.info($('Cannot find role definition with id '), id);
    }
  });
};
=======
>>>>>>> 932065d5d0aa9025fb051b5cf0ae057c0c555951
